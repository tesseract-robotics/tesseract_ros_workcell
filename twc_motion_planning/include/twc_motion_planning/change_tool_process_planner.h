...
{
class ChangeToolProcessPlanner : public tesseract_process_planners::ProcessPlanner
{
public:
  ChangeToolProcessPlanner(ProcessPlannerConfig config);
  ~ChangeToolProcessPlanner() override = default;

  void setToolPath(const std::vector<std::vector<tesseract_motion_planners::Waypoint::Ptr>>& tool_path) override;

  const tesseract_process_planners::ProcessDefinition& getProcessDefinition() const override;
